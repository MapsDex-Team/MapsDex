...
async def spawn(ctx: commands.Context[BallsDexBot], *, flags: SpawnFlags):
    ...
    if flags.tier_range:
        ...
        try:
            ...
            if len(parts) != 2:
                raise ValueError
            min_tier = int(parts[0].strip())
            max_tier = int(parts[1].strip())
        except ValueError:
            await ctx.send(
                "Invalid tier_range format. Use format like '1-100' for T1 to T100.",
                ephemeral=True
            )
            return

        enabled_collectibles = [x for x in countryballs.values() if x.enabled]
        ...
        max_possible_tier = max(rarity_to_collectibles.keys()) if rarity_to_collectibles else 1

        if min_tier < 1 or max_tier > max_possible_tier or min_tier > max_tier:
            await ctx.send(
                f"Invalid tier range. Must be between T1-T{max_possible_tier} and min <= max.",
                ephemeral=True
            )
            return

        filtered_balls = []
        for tier_idx in range(min_tier, max_tier + 1):
            filtered_balls.extend(rarity_to_collectibles.get(tier_idx, []))
